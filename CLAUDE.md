...
Also at SQLite startup, `internal/storage/factory.go` applies a fail-closed PRAGMA stanza: `journal_mode=WAL`, `synchronous=NORMAL`, `temp_store=MEMORY`, `wal_autocheckpoint=10000`, `journal_size_limit=67108864` (64 MB WAL cap), `busy_timeout=5000`, plus a **fixed 64 MB** page cache (`cache_size=-65536`) and `mmap_size=0`; `SQLITE_CACHE_SIZE_KB` and `SQLITE_MMAP_SIZE_BYTES` override both. They are sized to the #283 memory objective by the #292 read proof, not to the host: the pure-Go driver's page cache is **not** Go-heap memory (modernc's libc allocates it outside the heap, invisible to `GOMEMLIMIT`) and its allocator holds about **twice** the configured cache as anonymous RSS (277 MiB at the former 256 MB ceiling, 249 MiB at 128 MB, 125 MiB at 64 MB — page cache plus the sorter/temp space the wide-range reads use); an mmap window keeps every page a scan touches resident, bounded only by the window (formerly up to 1 GB), so it cannot sit inside a fixed RSS objective. The `membudget`-scaled sizing that preceded this (budget/32 and budget/8, clamped) is gone. `PRAGMA auto_vacuum=INCREMENTAL` is attempted best-effort **before** the WAL switch (the WAL header freezes the stored mode; only affects newly created DB files). Any pragma failure in the fail-closed stanza aborts startup with a wrapped error — these are not optional. See `docs/superpowers/specs/2026-05-24-mcp-7tool-sqlite-survival-design.md` for per-default reasoning.
...
Releases are draft-first: nothing is published until the exact candidate has been proven.

1. `scripts/release.sh vX.Y.Z[-pre] [--release] [--dry-run]` runs on a clean `main` that equals `origin/main`. It reads the live required status checks from main's branch protection and requires every context to report `success` for HEAD (check-runs plus commit statuses, so SonarCloud counts); a missing, pending, skipped, or failed context refuses with the context named. It then does a `CGO_ENABLED=0 go build` sanity check, creates the annotated tag, and pushes it once. `--release` also creates a **draft** GitHub release (`--prerelease` when the tag has a suffix) with generated notes and no artifacts. `--dry-run` performs every check and prints what would happen. A tag is never moved, deleted, or reused; a failed candidate gets a new version after a corrective commit.
2. `release.yml` runs at the tag ref (`push` of `v*`). Jobs, in order: `source identity` (protected-main ancestry, annotated tag, clean tree, required checks, Go version → `source-identity-v1.json`); `goreleaser` (GoReleaser OSS v2: linux/darwin × amd64/arm64, `CGO_ENABLED=0`, `-trimpath -s -w -X main.Version=<tag>`, `tar.gz` archives with LICENSE.md, README.md and the systemd unit files, sha256 `checksums.txt`, per-archive syft SBOMs, **cosign keyless** signature of `checksums.txt` as `.sig` + `.pem`; `release.draft: true` + `use_existing_draft: true` upload into the draft `release.sh` opened, or open one); `verify signed assets` (checksums, certificate, signature identity `release.yml@refs/tags/<tag>`, archive shape, target, version, embedded UI, SBOMs → `release-assets-v1.json`); then the exact-binary proofs `database lifecycle gate` (reusable `database-proof.yml`: `database proof · sqlite / postgres-16 / mysql-8.4 / sqlserver-2022`), `browser smoke · release binary`, `systemd proof · release archive`, and `linux arm64 smoke`; `release candidate manifest` (`release-candidate-v1.json`, uploaded as `release-candidate-<tag>-<sha>` with all evidence, 90 days); and `publish`, which un-drafts the release only when `limited_production.approved` is true.
3. Aggregate certification is `aggregate-release-gate.yml`, dispatched separately against the same signed Linux archive. It never blocks and is never blocked by publication; a failed aggregate run leaves aggregate mode available and unapproved.

Reruns dispatch `release.yml` at the tag ref (`gh workflow run release.yml --ref <tag>`): the `goreleaser` job is skipped and the run re-proves the existing draft assets. Assets are never rebuilt or replaced.

All third-party actions are SHA-pinned per Scorecard `Pinned-Dependencies`. $0/OSS-only: GoReleaser OSS, cosign (Apache-2.0), syft (Apache-2.0) — no Pro-only config keys. A published signed release is what makes Scorecard's **Packaging** and **Signed-Releases** checks score.
...
Metric gaps the gate documents on every run (closing them means touching `internal/`, which the gate tooling does not): `RecoveryStats.SkippedSeries` and `SeededBaselines` have no Prometheus gauge and are parsed from the server's slog line; `/api/metrics/*` carries no `truncated` field because the engine pages reads to completion; a drop counter vector that never fires emits nothing at all, so an empty `late_points_total` is indistinguishable from a deleted one without a witness metric; `test/aggprefill` emits no per-window observation totals; nothing reports logical charged bytes for the main tier. The process-RSS gap closed with #292: `otelcontext_process_resident_memory_bytes` (Linux, `/proc/self/statm`) and `otelcontext_go_heap_inuse_bytes` are sampled on scrape, and the read proof (`docs/gates/read/`) asserts the #283 RSS objectives per shape.
