...
func TestDefaultThresholdsMatchTheFrozenContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"main tier bytes", float64(th.DiskMainMaxBytes), 4.5 * 1024 * 1024 * 1024},
		{"aggregate tier bytes", float64(th.DiskAggregateMaxBytes), 1.5 * 1024 * 1024 * 1024},
		{"dlq tier bytes", float64(th.DiskDLQMaxBytes), 0.5 * 1024 * 1024 * 1024},
		{"wal tier bytes", float64(th.DiskWALTempTLSMaxBytes), 0.5 * 1024 * 1024 * 1024},
