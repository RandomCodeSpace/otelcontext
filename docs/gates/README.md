...
- `git`, `systemctl`, `systemd-run` and `taskset` installed in `/usr/bin`, `/bin`, `/usr/sbin`, `/sbin` or `/usr/local/bin`. The orchestrator resolves them from that fixed list and ignores `$PATH`: the provenance record depends on running the real `git` and the confinement teardown on the real `systemctl`, so a writable `$PATH` entry must not be able to substitute either. A tool installed only under `~/.local/bin` will not be found.
